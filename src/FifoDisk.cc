...
FifoDisk::FifoDisk(const std::string& classname, uint64_t size,
    ...
    held_bytes(0), held_pkts(0), oldestTimestamp(0), newestTimestamp(0), 
    classnameId(classnameId),
    filename_format(filename_format), classdir_format(classdir_format),
    dynlink_target_1(dynlink_target_1), dynlink_link_1(dynlink_link_1),
    dynlink_target_2(dynlink_target_2), dynlink_link_2(dynlink_link_2),
    dynlink_target_3(dynlink_target_3), dynlink_link_3(dynlink_link_3),
...
// Create or update soft link #1
void FifoDisk::update_dynlink(const char *target, const char *link, time_t newestTimestamp, const std::string &newCapDir, const std::string &newCapFname) {
  struct tm newestTimestampTM;
  if (localtime_r(&newestTimestamp, &newestTimestampTM) == NULL) {
    perror("localtime");
  }
  struct tm nowTM;
  ...
  if (localtime_r(&yesterday, &yesterdayTM) == NULL) {
    perror("localtime");
  }

  std::string target_path;
  target_path = fmt::format(target,
               fmt::arg("class_name", classname), 
               fmt::arg("class_id", classnameId), 
               fmt::arg("newest_timestamp", newestTimestampTM),
               fmt::arg("now_timestamp", nowTM),
               fmt::arg("yesterday_timestamp", yesterdayTM)
             );

  std::string link_path;
  link_path = fmt::format(link,
                fmt::arg("class_name", classname), 
                fmt::arg("class_id", classnameId), 
                fmt::arg("newest_timestamp", newestTimestampTM),
                fmt::arg("now_timestamp", nowTM),
  ...
  // also format/create directory path leading up to link_path as necessary
  // temp strip last component to get directory
  size_t fname_sep = link_path.rfind('/');
  if (fname_sep != std::string::npos) {
    link_path[fname_sep] = '\0';
    mkdirall(link_path.c_str());
    link_path[fname_sep] = '/';
  }

  char path[PATH_MAX+1];
  bool no_link = false;
  if (readlink(link_path.c_str(), path, sizeof(path)) < 0) {
    path[0] = '\0';
    no_link = true;
  }
  path[PATH_MAX] = '\0';

  // update link if it has changed
  if (target_path.compare(path) != 0) {
    if (!no_link) {
      if (unlink(link_path.c_str()) < 0) {
        std::string errmsg = "unlink: " + link_path;
        perror(errmsg.c_str());
      }
    }
    if (symlink(target_path.c_str(), link_path.c_str()) < 0) {
      std::string errmsg = "symlink: " + link_path + " to " + target_path;
      perror(errmsg.c_str());
    }
  } else {
    fprintf(stderr, "%s = %s, not updating %s\n", path, target_path.c_str(), link_path.c_str());
  }
}
    ...
    if (files.empty() ||
      ...
      if (!queries) {  /* no queries in progress at the moment */
        ...
        char *new_file_name;
          ...
          if (dynlink_target_1 != NULL && dynlink_link_1 != NULL) {  
            update_dynlink(dynlink_target_1, dynlink_link_1, newestTimestamp, dirpath, fname);
          }
          if (dynlink_target_2 != NULL && dynlink_link_2 != NULL) {  
            update_dynlink(dynlink_target_2, dynlink_link_2, newestTimestamp, dirpath, fname);
          }
          if (dynlink_target_3 != NULL && dynlink_link_3 != NULL) {  
            update_dynlink(dynlink_target_3, dynlink_link_3, newestTimestamp, dirpath, fname);
          }
